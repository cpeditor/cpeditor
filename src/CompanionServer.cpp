...
CompanionServer::CompanionServer(int port) {
  ...
  if (server->serverPort() == 0)
    Log::MessageLogger::error("Companion",
                              "Failed to listen to " + std::to_string(port) +
                                  " port. Is another process listening there?");
  else
    Log::MessageLogger::info(
        "Companion", "Server is listening on " +
...
void CompanionServer::onReadReady() {
  ...
  if (request.startsWith("POST") &&
    ...
    if (error.error == QJsonParseError::NoError) {
      ...
      payload.isInputstdin =
          doc["input"].toObject()["type"].toString() == "stdin";
      payload.isOutputstdout =
          doc["output"].toObject()["type"].toString() == "stdout";
      ...
      Log::MessageLogger::error(
          "Companion",
          "JSONParser reported errors. \n" + error.errorString().toStdString(),
          true);
    }

  } else {
    Log::MessageLogger::warn(
        "Companion", "An Invalid Payload was delivered on the listening port");
    socket->write("HTTP/1.1  OK\r\n");  // \r needs to be before \n
    socket->write("Content-Type: text/html\r\n");
    socket->write("Connection: close\r\n");
    socket->write("Pragma: no-cache\r\n");
    ...
    socket->write("<!DOCTYPE html>\r\n");
    socket->write(
        "<html><body><h1>Rejected!!! Only POST requests with Content-type JSON "
        "are allowed "
        "here.</h1></body></html>");
    socket->write(QByteArray());
    socket->disconnectFromHost();
