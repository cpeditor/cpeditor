...
- Help other people in issues/pull requests/Telegram group/other platforms if you are able to help them.
- Improve documentation, write tutorials and blogs on how to use CP Editor.
...
- Use Clang Format 9 (use `clang-format --version` to check your version, you may need to set the clang-format binary path for your IDE, for example, the default version of clang-format in VS Code may be clang-format-6) and `.clang-format` in the root directory to format your codes. It's OK if you failed to install Clang Format 9, because your codes will be automatically formatted after pushing to GitHub.
...
- How to add a setting?
  - You have to add the UI part in `ui/preferencewindow.ui`, add get/set functions in `include/Core/SettingsManager.hpp` and `src/Core/SettingsManager.cpp`, update settings and probably extract setting to UI in `src/preferencewindow.cpp`.
- Some files are missing when compiling.
  - This is probably because the submodules are updated if the CI tests are passed. You can run the following commands:
    ...
    Or you can even delete the repo on your disk and clone again.
